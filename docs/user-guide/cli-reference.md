...
| Flag | Default | What |
|---|---|---|
| `--advantage-estimator` | `grpo` | `grpo`, `gspo`, `rloo`, `ppo`, `reinforce_plus_plus`, `reinforce_plus_plus_baseline`. RLOO uses the other rollouts in each prompt group as its baseline. On-policy distillation is not an estimator — enable it with `--use-opd` on top of any of these. |
...
| Flag | Type | Default | Notes |
|---|---|---|---|
| `--advantage-estimator` | enum | `grpo` | `grpo`, `gspo`, `rloo`, `ppo`, `reinforce_plus_plus`, `reinforce_plus_plus_baseline`. |
